...
// build/dev/javascript/lustreofheroes/lustreofheroes.mjs
var Dashboard = class extends CustomType {
};
var MyHeroes = class extends CustomType {
};
var Hero = class extends CustomType {
  constructor(name) {
    super();
    this.name = name;
  }
...
var Model = class extends CustomType {
  constructor(name, heroes, current_route) {
    super();
    this.name = name;
    this.heroes = heroes;
...
var UserAddedHero = class extends CustomType {
};
function update2(model, msg) {
  if (msg instanceof UserUpdatedName) {
    let name = msg[0];
    return [model.withFields({ name }), none()];
  } else if (msg instanceof UserAddedHero) {
    let $ = length2(model.name);
    if ($ === 0) {
      return [model, none()];
    } else {
      return [
        model.withFields({
          name: "",
          heroes: prepend(new Hero(model.name), model.heroes)
        }),
        none()
      ];
    }
...
function on_route_change(uri) {
  ...
  if ($.hasLength(1) && $.head === "my-heroes") {
    return new OnRouteChange(new MyHeroes());
  } else {
    return new OnRouteChange(new Dashboard());
  }
}
function init3(_) {
  return [
    new Model(
      "",
      toList([new Hero("Batman"), new Hero("Superman")]),
      new Dashboard()
    ),
  ...
  return ul(
    ...
    (() => {
      ...
      let _pipe$2 = map3(
        _pipe$1,
        (hero) => {
          return li(toList([]), toList([text2(hero.name)]));
        }
      );
      return to_list(_pipe$2);
    })()
...
function view_dashboard(model) {
  return centre2(
    ...
    stack2(
      toList([]),
      toList([
        h1(toList([]), toList([text2("Top heroes")])),
        (() => {
          let _pipe = model.heroes;
          let _pipe$1 = top_heroes(_pipe);
          return heroes_to_html_list(_pipe$1);
        })(),
        input3(
          toList([
            value(model.name),
        ...
        button3(
          toList([on_click(new UserAddedHero())]),
          toList([text2("Add hero")])
        )
      ])
    )
  );
}
function view_my_heroes(model) {
  return div(toList([]), toList([heroes_to_html_list(model.heroes)]));
}
function view(model) {
  ...
  let page = (() => {
    ...
    if ($ instanceof Dashboard) {
      ...
      return view_my_heroes(model);
    }
  })();
  return stack2(toList([style(styles)]), toList([header2(), page]));
}
function main() {
